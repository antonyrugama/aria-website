...
undeclared filter before a pane sees one, so no mutation of a pane can turn that claim red, and
it is held where it can fail, in `scripts/ops-shell-pane-v2.test.mjs`. Cloud costs is the single exclusion, because it is
