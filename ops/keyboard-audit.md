...
| Tab stops recorded | 352 |
| Interactive controls found | 315 |
| Controls never reached by Tab | **0** |
| Focus traps | **0** |
| Walks whose Shift+Tab exactly retraces Tab | 20/20, **312 stops retraced** over 330 Shift+Tab presses |
...
- **No focus traps.** 20 walks, 352 stops, 0 traps. A control is called a trap only after **12** consecutive Tab presses leave `document.activeElement` unchanged — twice the widest composite input Chrome ships, which is the 6-field `datetime-local`.
- **Nothing unreachable.** 315 enabled, visible, interactive controls; 0 were not reached by Tab.
...
- **Shift+Tab is the exact inverse of Tab** on 20 of 20 walks, over the WHOLE walk rather than a prefix of it: 312 stops retraced against 352 forward stops, which took 330 presses because a composite input consumes several. 0 walks made no press and are counted on neither side.
